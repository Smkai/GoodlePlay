...
public class GameFragment extends BaseAppListFragment {
    ...
    protected void startLoadData() {
        Call<List<AppListBean>> listCall = HeiMaRetrofit.getInstance().getApi().listGame(0);
    ...
    protected void starLoadMoreData() {
        Call<List<AppListBean>> listCall = HeiMaRetrofit.getInstance().getApi().listGame(mDataList.size());
        listCall.enqueue(new Callback<List<AppListBean>>() {
            @Override
            public void onResponse(Call<List<AppListBean>> call, Response<List<AppListBean>> response) {
                getDataList().addAll(response.body());
